...
public class FlowBehaviourEditor : Editor
{
        ...
        if (paiting)
        {
            ...
            for (int i = 0; i < behaviour.Fluids.Length; i++)
            {
                ...
                if (falloff > 0)
                {
                    ...
                    {
                        // 修复：使用 ArrowHandleCap 替代 ArrowCap
                        Handles.ArrowHandleCap(0, behaviour.Fluids[i].world + Vector3.up * 0.1f, Quaternion.LookRotation(behaviour.Fluids[i].fluidVector), behaviour.PainterSize / 2, EventType.Repaint);
                    }
                }
                else
                {
    ...
    void DrawDefaultMesh()
    {
        /*for(int i=0;i<_behaviour._fluids.Length;i++)
		{
			...
			// 修复：使用 ArrowHandleCap 替代 ArrowCap
			Handles.ArrowHandleCap(0,_behaviour._fluids[i].world,Quaternion.LookRotation(_behaviour._fluids[i].fluidVector),0.5f, EventType.Repaint);
